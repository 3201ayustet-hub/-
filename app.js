...
let hudImage = null;
let queue = JSON.parse(localStorage.getItem("v4_queue") || "[]");
let players = JSON.parse(localStorage.getItem("v4_players") || "[]");
...
function updateTelopText(){
  ...
  let text = "";
  if(side==="both" && action==="チェック") text = "お互いチェックしリバーへ";
  else if(["セット完成","フラッシュ完成","フルハウス完成"].includes(action)) text = actor ? `${actor}、${action}` : action;
  else if(result && (action==="勝利" || action==="敗北")) text = `${p}${result}`;
  else if(result && result.includes("負け")) text = `${p}${result}`;
  else if(action==="オールイン") text = amount ? `${p}${amount}点オールイン` : `${p}オールイン`;
  else if(needsAmount && amount) text = `${p}${amount}点${action}`;
  else text = actor ? `${actor}が${action}` : action;
...
function styleForTelop(){
  ...
  if(action==="チェック") return {key:"light", hue:0, tilt:0, font:'900 {size}px "Hiragino Sans","Yu Gothic",sans-serif'};
  if(side==="opponent" && ["ベット","レイズ","3ベット","4ベット"].includes(action)) return {key:"fear", hue:285, tilt:-5, font:'900 {size}px "Arial Black","Yu Gothic",sans-serif'};
  if(side==="opponent" && action==="オールイン") return {key:"alarm", hue:350, tilt:-6, font:'900 {size}px Impact,"Arial Black",sans-serif'};
  if(side==="self" && ["ベット","レイズ","3ベット","4ベット"].includes(action)) return {key:"fight", hue:18, tilt:-3, font:'900 {size}px Impact,"Arial Black","Hiragino Sans",sans-serif'};
  if(side==="self" && action==="オールイン") return {key:"allin", hue:12, tilt:-4, font:'900 {size}px "Arial Black","Hiragino Sans","Yu Gothic",sans-serif'};
  if(action.includes("完成") || result.includes("セット") || result.includes("フルハウス") || action==="勝利") return {key:"gold", hue:42, tilt:-3, font:'900 {size}px Georgia,"Hiragino Mincho ProN","Yu Mincho",serif'};
  if(action==="敗北" || result.includes("負け") || result.includes("敗北")) return {key:"lose", hue:310, tilt:-5, font:'900 {size}px "Arial Black","Yu Gothic",sans-serif'};
  if(action==="コール") return {key:"cool", hue:195, tilt:-1, font:'900 {size}px "Trebuchet MS","Hiragino Sans",sans-serif'};
  return {key:"normal", hue:22, tilt:-2, font:'900 {size}px "Arial Black","Hiragino Sans",sans-serif'};
}
...
function renderTelop(){
  ...
  canvas.width=1500; canvas.height=260;
  ...
  const fontSize=fitFontSize(text, st.font, 140, canvas.width-120);
  ctx.save();
...
function drawTelopFx(w,h,key,p){
  if(key==="light") return;
  const cx=w/2,cy=h/2;
  const g=ctx.createRadialGradient(cx,cy,5,cx,cy,520);
  g.addColorStop(0,hsla(p.accent,.22)); g.addColorStop(.42,hsla(p.main,.11)); g.addColorStop(1,"rgba(0,0,0,0)");
  ctx.fillStyle=g; ctx.fillRect(0,0,w,h);

  const count= key==="allin" || key==="alarm" ? 72 : key==="fear" ? 44 : 32;
  for(let i=0;i<count;i++){
    const a=Math.random()*Math.PI*2, len=80+Math.random()*260;
    ctx.globalAlpha= key==="light" ? .05 : .18;
    ctx.strokeStyle=i%2?p.accent:p.main;
    ctx.lineWidth=2+Math.random()*8;
    ctx.beginPath();
    ctx.moveTo(cx+Math.cos(a)*70, cy+Math.sin(a)*18);
    ctx.lineTo(cx+Math.cos(a)*len, cy+Math.sin(a)*len*.32);
    ctx.stroke();
  }
  ctx.globalAlpha=1;
}
...
function drawTelopText(text,x,y,font,p){
  ...
  ctx.strokeStyle="#070707"; ctx.lineWidth=26; ctx.strokeText(text,x,y);
  ctx.strokeStyle="#fff3c8"; ctx.lineWidth=12; ctx.strokeText(text,x,y);
  ...
  ctx.save(); ctx.globalAlpha=.3; ctx.fillStyle="#fff"; ctx.fillText(text,x,y-24); ctx.restore();
}

function renderHud(){
  $("previewTitle").textContent="プレイヤーHUDプレビュー";
  $("previewWrap").className="previewWrap hudPreview";
  canvas.width=1200; canvas.height=330;
  ctx.clearRect(0,0,canvas.width,canvas.height);
  drawHudBase(40,55,1110,220,["#260603","#821509","#ff3b1f"]);
  drawAvatar(58,28,270,"#ff3b1f",hudImage);
  drawCards(370,25,$("hRank1").value,$("hSuit1").value,$("hRank2").value,$("hSuit2").value,true);
  drawHudLabels(
    $("hudName").value || "PLAYER",
    $("hudPos").value,
    $("hudAct").value,
    formatAmount($("hudAmount").value),
    "#fff6ca"
  );
}
...
function renderEnemy(){
  ...
  $("previewWrap").className="previewWrap hudPreview";
  canvas.width=1200; canvas.height=310;
  ctx.clearRect(0,0,canvas.width,canvas.height);
  drawHudBase(40,55,1110,200,["#08090d","#25262d","#dfe6f3"]);
  drawAvatar(62,35,230,"#dfe6f3",null);
  const mode=$("enemyCardMode").value;
  if(mode==="back") drawCards(350,25,"A","♠","K","♠",false);
  if(mode==="select") drawCards(350,25,$("eRank1").value,$("eSuit1").value,$("eRank2").value,$("eSuit2").value,true);
  drawHudLabels(
    $("enemyName").value || "プレイヤー",
    $("enemyPos").value,
    $("enemyAct").value,
    formatAmount($("enemyAmount").value),
    "#ffffff"
  );
}
...
function drawHudBase(x,y,w,h,colors){
  ctx.save();
  ctx.shadowColor=colors[2]; ctx.shadowBlur=18;
  ...
  for(let i=0;i<9;i++){
    ...
    ctx.beginPath(); ctx.moveTo(x+w*.48+i*28,y+15); ctx.lineTo(x+w-25,y+h-22-i*4); ctx.stroke();
  }
  ctx.restore();
}
...
function drawCards(x,y,r1,s1,r2,s2,face){
  if(face){ drawCard(x,y,r1,s1); drawCard(x+106,y,r2,s2); }
  else { drawBack(x,y); drawBack(x+106,y); }
}

function drawCard(x,y,r,s){
  const red=s==="♥"||s==="♦";
  ctx.save();
  roundRect(ctx,x,y,92,126,9); ctx.fillStyle="#fffdf6"; ctx.fill(); ctx.lineWidth=4; ctx.strokeStyle="#2d1a14"; ctx.stroke();
  ctx.fillStyle=red?"#d80e12":"#080808";
  ctx.font='900 38px "Arial Black",sans-serif'; ctx.textAlign="left"; ctx.textBaseline="top";
  ctx.fillText(r,x+10,y+8);
  ctx.font='900 34px serif'; ctx.fillText(s,x+12,y+49);
  ctx.font='900 52px Georgia,serif'; ctx.textAlign="center"; ctx.fillText(r,x+46,y+65);
  ctx.restore();
}

function drawBack(x,y){
  ctx.save();
  roundRect(ctx,x,y,92,126,9);
  const g=ctx.createLinearGradient(x,y,x+92,y+126); g.addColorStop(0,"#3b3b42"); g.addColorStop(1,"#070707");
  ctx.fillStyle=g; ctx.fill(); ctx.lineWidth=4; ctx.strokeStyle="#aaa"; ctx.stroke();
  ctx.globalAlpha=.28; ctx.beginPath(); ctx.arc(x+46,y+63,28,0,Math.PI*2); ctx.strokeStyle="#eee"; ctx.lineWidth=5; ctx.stroke();
  ctx.restore();
}
...
function drawHudLabels(name,pos,act,amount,accent){
  ...
  ctx.textAlign="left";
  ctx.font='900 58px "Hiragino Sans","Yu Gothic",sans-serif';
  outlined(name,610,140,"#fff");
  ctx.font='900 50px "Arial Black","Hiragino Sans",sans-serif';
  outlined(`${act} ${amount}`.trim(),610,210,accent);
  ctx.textAlign="right";
  ctx.font='900 68px "Arial Black",sans-serif';
  outlined(pos,1090,125,"#fff");
}

function outlined(text,x,y,fill){
  ctx.strokeStyle="#050505"; ctx.lineWidth=10; ctx.strokeText(text,x,y);
...
function savePlayerTemplate(){
  ...
  const data = {
    name,
    image: hudImage ? hudImage.src : "",
    savedAt: Date.now()
  };
  players = players.filter(p=>p.name!==name);
  players.push(data);
  localStorage.setItem("v4_players", JSON.stringify(players));
  updateTemplateSelect();
  renderPlayerList();
  alert("テンプレ保存しました");
}
...
function renderPlayerList(){
  ...
  root.querySelectorAll("[data-del]").forEach(b=>b.onclick=()=>{ players.splice(Number(b.dataset.del),1); localStorage.setItem("v4_players",JSON.stringify(players)); updateTemplateSelect(); renderPlayerList(); });
}

function getCurrentItem(){
  if(currentTab==="telop") return {type:"telop", title:$("tText").value, state:{
    tText:$("tText").value, tAction:$("tAction").value, tSide:$("tSide").value, selfName:$("selfName").value, tAmount:$("tAmount").value, tResult:$("tResult").value
  }};
...
function collectHudState(){
  return {
    name:$("hudName").value, pos:$("hudPos").value, act:$("hudAct").value, amount:$("hudAmount").value,
    r1:$("hRank1").value,s1:$("hSuit1").value,r2:$("hRank2").value,s2:$("hSuit2").value,
    image: hudImage ? hudImage.src : ""
  };
}
function collectEnemyState(){
  return {
    name:$("enemyName").value,pos:$("enemyPos").value,act:$("enemyAct").value,amount:$("enemyAmount").value,
...
function addToQueue(){
  ...
  queue.push(item);
  localStorage.setItem("v4_queue",JSON.stringify(queue));
  renderQueue();
  alert("キューに追加しました");
}
...
function saveQueue(){ localStorage.setItem("v4_queue",JSON.stringify(queue)); renderQueue(); }
...
function quick(k){
  const p={
    selfRaise:["self","ロクヤマ","2,400","レイズ",""],
    oppBet:["opponent","ロクヤマ","76,000","ベット",""],
    selfAllin:["self","ロクヤマ","193,000","オールイン",""],
    oppAllin:["opponent","ロクヤマ","76,000","オールイン",""],
    set:["self","ロクヤマ","","セット完成","Jのセット"],
    bothCheck:["both","ロクヤマ","","チェック",""],
    lose:["opponent","ロクヤマ","","敗北","フラッシュで負け"]
  }[k];
  if(!p) return;
